...
**"哪个目录算大"不能靠猜**——名字、位置都不可靠。grep_guard 在拦截前对搜索根目录做**带配额的实时文件计数探测**（条目上限按工具区分：grep 系 10000、rg/find 100000；或 2 秒时间预算，先到即停），数超限才拦截。小目录完全无感放行。**探测是深度感知的**：命令带 `-maxdepth N` / `--max-depth N` 时只在限定深度内计数——`find 巨目录 -maxdepth 1` 这类浅层快速搜索不会被误拦。
